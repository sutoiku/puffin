...
**Note**: This process can be further optimized by having serverless functions report that most values are distinct within their respective partitions. In that case, we know that we have duplicates, but we also know that such duplicates are likely to be few in relation to the total number of values in the column. In that case, we can still implement the algorithm described for scenario `#1` in order to accurately identify unique values and detect possible duplicate values.
