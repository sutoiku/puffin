...
Several options are currently being considered for the packaging of the Query Generator. One of them is to use [WebAssembly](https://webassembly.org/), which would allow both client-side and cloud-side execution. This meta-extension mechanism would also make it easier to develop some DuckDB extensions, without having to compile them for different platforms.
