...
## Excel Client
DuckDB is being natively integrated within more and more client-side applications, and PuffinDB is not interested in developing any native clients and any extensions to client-applications (these will become everywhere very soon). Nevertheless, no add-in for Excel is available at this time, and PuffinDB will develop one if necessary. Proper support of DuckDB from Excel is a must.
